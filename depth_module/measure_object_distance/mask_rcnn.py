...
class MaskRCNN:
    def __init__(self):
        ...
        # Loading Mask RCNN
        self.net = cv2.dnn.readNetFromTensorflow("/home/pc/Volkswagon/Summer_project/measure_object_distance/dnn/frozen_inference_graph_coco.pb",
                                            "/home/pc/Volkswagon/Summer_project/measure_object_distance/dnn/mask_rcnn_inception_v2_coco_2018_01_28.pbtxt")
        ...
        self.classes = []
        with open("/home/pc/Volkswagon/Summer_project/measure_object_distance/dnn/classes.txt", "r") as file_object:
    ...
    def draw_object_info(self, bgr_frame, depth_frame):
        ...
        for box, class_id, obj_center in zip(self.obj_boxes, self.obj_classes, self.obj_centers):
            ...
            if len(self.my_deque) >= self.max_size:
                self.my_deque.popleft()  # Remove the leftmost element if the deque is full
            self.my_deque.append(depth_mm)
            
            min_dep=min(self.my_deque)
            
            self.res=min(min_dep,self.res)
            # self.result(self.res)
            self.temp=self.res
            # print("depth: ",self.res)
            self.res=10000
            ...
            cv2.putText(bgr_frame, "{} m".format(depth_mm / 1000), (x + 5, y + 60), 0, 1.0, (255, 255, 255), 2)
            cv2.rectangle(bgr_frame, (x, y), (x2, y2), color, 1)




        return bgr_frame,self.temp
